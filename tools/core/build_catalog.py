...
def build_catalog(downloads_dir: str, output_dir: str, skip_images: bool = False, skip_seo: bool = False):
    """
    ...
    if not skip_seo:
        try:
            ...
            print("\n🌐 Auto-generating SEO static pages & sitemap...")
            generate_all_pages(str(catalog_file), str(output_path))
            generate_sitemap(str(catalog_file), str(output_path))
            print("✓ SEO pages & sitemaps updated successfully.")
        except Exception as e:
            print(f"Notice: SEO auto-generation skipped or encountered error: {e}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", default=".", help="Path to output website folder")
    parser.add_argument("--skip-images", action="store_true", help="Skip webp image generation")
    parser.add_argument("--skip-seo", action="store_true", help="Skip static SEO landing page generation")
    args = parser.parse_args()

    build_catalog(args.downloads, args.output, args.skip_images, args.skip_seo)
